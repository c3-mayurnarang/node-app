pipeline {
  ...
  stages {
    ...
    stage('Create Packer AMI') {
        steps {
          withCredentials([
            usernamePassword(credentialsId: 'aws-test-account-test', passwordVariable: 'AWS_SECRET', usernameVariable: 'AWS_KEY')
          ]) {
    ...
    stage('AWS Deployment') {
      steps {
          withCredentials([
            usernamePassword(credentialsId: 'aws-test-account-test', passwordVariable: 'AWS_SECRET', usernameVariable: 'AWS_KEY'),
            usernamePassword(credentialsId: 'aws-test-account-test', passwordVariable: 'REPO_PASS', usernameVariable: 'REPO_USER'),
          ]) {
